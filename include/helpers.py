...
def play_dataset_sample(dataset_row, audio_path):
    ...
    # Sound preview
    return IPython.display.Audio(file_path)
...
def plot_train_history_orig(history, x_ticks_vertical=True):
    ...
    '''
    plt.legend(['Train', 'Test', ('%.3f' % min_loss), ('%.3f' % min_val_loss)], 
                loc='upper right', fancybox=True, framealpha=0.9,  shadow=True, borderpad=1)
    '''
    plt.legend(['Train', 'Test'], loc='upper right', fontsize=16, framealpha=0.9,  shadow=True, borderpad=1)
    plt.xticks(np.arange(0, len(history['loss']), 20.0),fontsize=20)
    ...
    plt.xticks(np.arange(0, len(history['accuracy']), 20.0),fontsize=20)
    plt.yticks(fontsize=20)

    plt.show()


def plot_train_history(history, x_ticks_vertical=True):
    history = history.history
    ...
    plt.plot(history['loss'], color="r", alpha=0.9)
    plt.plot(history['val_loss'], color="b", linestyle="--")
    #plt.plot(min_loss_x, min_loss, marker='o', markersize=3, color="#1f77b4", alpha=0.7, label='Inline label')
    #plt.plot(min_val_loss_x, min_val_loss, marker='o', markersize=3, color="#ff7f0e", alpha=7, label='Inline label')
    #plt.title('Model loss', fontsize=20)
    plt.ylim(0,0.25)
    ...
    plt.legend(['Train', 'Test', ('%.3f' % min_loss), ('%.3f' % min_val_loss)], 
                loc='upper right', fancybox=True, framealpha=0.9,  shadow=True, borderpad=1)
    '''
    plt.legend(['Train', 'Test'], loc='upper right', fontsize=35 ) # framealpha=0.9,  shadow=True, borderpad=1)
    plt.xticks(np.arange(0, len(history['loss']), 10.0),fontsize=35)
    plt.yticks(fontsize=30)
    plt.savefig(r'D:\KISTI\Project\052020-Traffic-Sound\Yi-Source-Code-3\output/fig_loss.png')
    plt.show()

    # summarize history for accuracy, display max
    plt.figure(figsize=(20,11))
    plt.plot(history['accuracy'], color="r", alpha=0.9)
    plt.plot(history['val_accuracy'], color="b", linestyle="--")
    #plt.plot(max_accuracy_x, max_accuracy, marker='o', markersize=3, color="#1f77b4", alpha=7)
    #plt.plot(max_val_accuracy_x, max_val_accuracy, marker='o', markersize=3, color="orange", alpha=7)
    #plt.title('Model accuracy', fontsize=20)
    plt.ylabel('Accuracy', fontsize=35)
    plt.xlabel('Epoch', fontsize=35)
    plt.legend(['Train', 'Test'], loc='lower right', fontsize=35 ) 
    plt.figure(num=1, figsize=(10, 6))
    plt.xticks(np.arange(0, len(history['accuracy']), 10.0),fontsize=30)
    plt.ylim(0.2,1.1)
    plt.yticks(fontsize=30)
    plt.savefig(r'D:\KISTI\Project\052020-Traffic-Sound\Yi-Source-Code-3\output/fig_acc.png')
    plt.show()
...
def plot_confusion_matrix(cm,
    ...
    ax.set_xlabel('Predicted label', fontsize = 18) #xlabel
    ax.set_ylabel('True label', fontsize = 18)#ylabel     
    ax.set_title(title,fontsize = 18)#ylabel     

    # Rotate the tick labels and set their alignment.
    #plt.setp(ax.get_xticklabels(), rotation=45, ha="right", fontsize=12,rotation_mode="anchor")

    plt.setp(ax.get_xticklabels(), rotation=0, ha="right", fontsize=18,
    ...
    plt.ylim(-0.5,5.5)
    plt.ylim(5.5,-0.5)
    plt.savefig('./output/fig_confusionMatrix.png')
    plt.show()